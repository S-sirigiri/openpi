...
Inputs the cost uses:
    - actions: (..., horizon, action_dim) still in the normalized delta space
    - current_joint_pos: (..., 7) absolute, in radians, the proprio state
    - norm stats for the first 7 action dims
Output:
    - world_path: (..., horizon, 3) EE positions along the predicted horizon
"""
...
def build_world_path(
    ...
    Args:
        ...
        action_mean_7: (7,) action normalization mean, first 7 dims.
        action_std_7: (7,) action normalization std, first 7 dims.
        base_world_T: (4,4) robot base pose in world frame.
        ee_offset_T: (4,4) extra fixed offset applied after the flange.
    ...
    # Broadcast current joint state across the horizon then add deltas.
    abs_joints = arm_deltas + current_joint_pos[..., None, :]

    # Vectorize FK over (B, H) -> (B, H, 3).
